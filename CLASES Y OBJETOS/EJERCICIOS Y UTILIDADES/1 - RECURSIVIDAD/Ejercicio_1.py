...
# Debes definir una función llamada torres_de_hanoi(n, origen, destino, auxiliar) que, dado el número total de discos n y las torres de origen, destino y auxiliar, 
# imprima los pasos necesarios para lograr el movimiento de todos los discos desde la torre A hasta la torre C.

# A continuacion un ejemplo de una posible entrada y salida de la solucion:

def moviendo_disco(origen, destino, disco):
    print(f"Moviendo disco {disco} de la columna {origen} hasta la columna {destino}")


def torres_de_hanoi(n, origen, destino, auxiliar):
    # Caso base // Recursividad
    if n == 1:
        moviendo_disco(origen, destino, n)
        return
